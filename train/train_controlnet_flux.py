...
def log_validation(
    ...
    pipe_dtype = torch.float32 if torch.backends.mps.is_available() else torch.bfloat16
    ...
    if not is_final_validation:
        ...
        load_kwargs = {"torch_dtype": pipe_dtype}
        if args.save_weight_dtype != "fp32":
            load_kwargs["variant"] = args.save_weight_dtype
    ...
    if torch.backends.mps.is_available():
        logger.info("Skipping validation on MPS (dtype mismatch in pipeline inference)")
        return []
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(flux_controlnet):
                ...
                cn_dtype = torch.float32 if torch.backends.mps.is_available() else weight_dtype
                controlnet_block_samples, controlnet_single_block_samples = flux_controlnet(
                    hidden_states=noisy_model_input.to(dtype=cn_dtype),
                    controlnet_cond=control_image.to(dtype=cn_dtype),
                    timestep=timesteps / 1000,
                    guidance=guidance_vec.to(dtype=cn_dtype) if guidance_vec is not None else None,
                    pooled_projections=batch["unet_added_conditions"]["pooled_prompt_embeds"].to(dtype=cn_dtype),
                    encoder_hidden_states=batch["prompt_ids"].to(dtype=cn_dtype),
                    txt_ids=batch["unet_added_conditions"]["time_ids"][0].to(dtype=cn_dtype),
                    img_ids=latent_image_ids.to(dtype=cn_dtype),
                    return_dict=False,
                )
